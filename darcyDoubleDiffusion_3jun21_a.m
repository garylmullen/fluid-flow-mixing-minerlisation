...
runID = 'test_DoubleDiff_3jun21_b';
...
C0    = 0.00000001;     % top concentration  [C]
...
f = f0 + df.*rn; %+(f1-f0) .* Z/D ;  %porosity    (linear decrease + random perturbation)
...
while time <= tend
    ...
    if ~mod(m,nop)
       ...
       %The solves the issue. Now need to define the size of the figure
       fh2=figure('Visible','off');
        
       
        % having fh2.Visible = 'off';means figure pops up then closes, but still
        % interrupts the desktop
        %fh2.Visible = 'off';
                      
        sgtitle(sprintf('Time elapsed %.1f years', time/31557600))
        subplot(2,3,1);
        imagesc(x,z,-w.*3600*24*365.25); axis equal tight; colorbar;
        title('Segregation','z-speed [m/yr]')% Split the title to make it fit
        subplot(2,3,2);
        ...
        imagesc(x,z,f); axis equal tight; colorbar;
        title('Porosity [vol]')
        %Having draw now on or off seems to make no difference
        drawnow  
       
        
        %No Print means no images written to folder    
